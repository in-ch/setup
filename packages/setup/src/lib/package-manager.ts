...
export const installEslintDependencies = async (configType: string): Promise<void> => {
  const dependencyMap = {
    'import-sort': 'eslint @types/eslint eslint-plugin-jsdoc eslint-plugin-no-for-of-array eslint-plugin-vue @eslint/js typescript-eslint globals',
    'airbnb': 'eslint eslint-config-airbnb-base eslint-plugin-import @typescript-eslint/parser @typescript-eslint/eslint-plugin',
